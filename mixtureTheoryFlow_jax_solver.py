...
# Below this volume fraction, a phase is treated as having collapsed --
# its velocity/friction are no longer computed via division by its own
# (near-vanished) volume fraction, since that amplifies noise into large,
# unstable values. Instead it is treated as moving with the dominant phase.
# See time_step_learned / time_step (velocity) and advance_momentum (friction).
#PHI_COLLAPSE = 1e-3
PHI_COLLAPSE = 0.1
#PHI_COLLAPSE = 0.075
...
def make_grid(L, N):
    ...
    L : dimensionless pipe length, L* = L/D
    N : int   — number of cells

    Returns dx* (cell width) and x* (cell center coordinates), both
    in units of D. See nondimensional convention note in advance_mass.
    """
    dx = L / N
...
def initial_conditions(x, L, phi1_0, rho1, rho2, p_inlet, p_outlet):
    ...
    x        : (N,) dimensionless cell center coordinates, x* = x/D
    L        : dimensionless pipe length, L* = L/D
    phi1_0   : initial volume fraction of phase 1 (uniform) -- unchanged
    rho1     : rho1* = 1.0 exactly (reference density)
    rho2     : rho2* = rho2/rho1 (e.g. ~0.83 for Ibarra)
    p_inlet  : dimensionless inlet pressure, p* = p/(rho1*Um^2)
    p_outlet : dimensionless outlet pressure, p* = p/(rho1*Um^2)

    Returns a dict of dimensionless state arrays, each shape (N,).
    """
    N = len(x)
...
def compute_dt(state, dx, Re1, Re2, cfl=0.4, dt_max=1e-4):

    """
    Compute the maximum stable DIMENSIONLESS time step from the CFL
    condition. Wave speeds are the phase velocities plus pressure wave
    speed. For incompressible phases we use max(|u1|, |u2|) as a proxy.

    dx, dt_max : dimensionless, dx* = dx/D, dt_max* = dt_max_seconds*Um/D
    Re1, Re2   : reference Reynolds numbers, Re_i = rho1*Um*D/mu_i --
                 computed PER CONDITION (see advance_mass nondimensional
                 convention note), not global constants, since Um varies
                 by experimental condition rather than being fixed.
    """
    eps_phi = 1e-6
    ...
    # Viscous stability: nu_i* = mu_i/(rho1*Um*D) = 1/Re_i exactly, so
    # the dimensionless kinematic viscosity is just 1/Re -- no separate
    # mu or rho_val needed here, unlike the dimensional version.
    nu_max_star = jnp.maximum(1.0 / Re1, 1.0 / Re2)
    dt_visc = 0.5 * dx**2 / nu_max_star
...
#
# NONDIMENSIONAL CONVENTION: every quantity below is expected to already be
# dimensionless, scaled by reference values L_c=D, U_c=Um, t_c=D/Um, rho_c=rho1:
#   phi1, phi2  -- unchanged, already dimensionless (volume fractions)
#   rho1, rho2  -- rho1* = rho1/rho1 = 1.0 exactly; rho2* = rho2/rho1 (~0.83)
#   u1, u2      -- u* = u/Um
#   dx, dt      -- dx* = dx/D, dt* = dt*Um/D
# The formulas below are UNCHANGED from the dimensional version -- every term
# is built purely from ratios of these rescaled quantities, so the dimensional
# prefactors cancel exactly and no line needs to change.
...
def advance_mass(state, dt, dx):
    ...
    Operates on dimensionless quantities -- see the nondimensional
...
def advance_momentum(state, dt, dx, drag_coeff, d_b, D, mu1, mu2, theta,
                      w=1.0, k1=1.1102, k2=0.5853, Re1_ref=None, Re2_ref=None, Fr_ref=None):
#def advance_momentum(state, dt, dx, drag_coeff, d_b, D, mu1, mu2, theta, w=1.0, k1=1.0, k2=1.0):
    """
    k1=1.1102  k2=0.5853
    ...
    Re1_ref, Re2_ref: reference Reynolds numbers, Re_i_ref = rho1*Um*D/mu_i,
    computed PER CONDITION from that condition's own Um (see compute_dt /
    generate_real_dataset notes). Used in the viscous stress term and to
    rescale the local Blasius/laminar Reynolds numbers below.
    Deliberately NOT named Re1/Re2: those names are already used further
    down for the LOCAL, per-cell Reynolds numbers feeding the wall-
    friction switch -- a different, position-varying quantity. Reusing
    the name here would silently shadow this argument with that local
    variable.

    Fr_ref: reference Froude number, Fr_ref = Um/sqrt(g*D), also computed
    PER CONDITION. Currently only affects the gravity term, which is
    exactly zero whenever theta=0 (every dataset in this project so far).

    Re1_ref/Re2_ref/Fr_ref default to None only so old dimensional call
    sites still parse -- calling with the defaults will raise, since
    None cannot be used in the arithmetic below. mu1/mu2 are still
    accepted but are now unused inside this function (left in place to
    avoid touching every caller in this same step).
    """
    phi1 = state['phi1']
    phi2 = state['phi2']
    rho1 = state['rho1']
    ...
    visc1 = phi1_int * (1.0 / Re1_ref) * d2u1_dx2
    visc2 = phi2_int * (1.0 / Re2_ref) * d2u2_dx2
    
    # --- Gravity Term ---
    # 1/Fr_ref**2 replaces the raw dimensional g=9.81 -- see the
    # nondimensional convention note in advance_mass / this function's
    # docstring above. Currently zero on every dataset in this project
    # (theta=0, so sin(theta)=0), but kept dimensionally correct.
    ...
    g_x = -(1.0 / Fr_ref**2) * jnp.sin(theta)   # negative because gravity opposes upward flow
    ...
    #'''
    # --- Wall friction (Darcy-Weisbach) -- DISABLED: replaced with
    # per-phase Blasius friction below, to capture phase-inversion-
    # dependent behavior (see conversation notes / CLAUDE.md).
    ...
    # ENABLED: replaces the mixture-averaged friction above. Each phase's
    # friction now scales with its own (volume-fraction-scaled) hydraulic
    # diameter, so the minority phase is disproportionately penalized --
    # this can flip which phase ends up faster as the minority/majority
    # phase identity swaps around phase inversion.
    ...
    # Safe substitution: never let a near-vanished phase's volume fraction
    # reach the hydraulic-diameter division below, even in a jnp.where
    # branch that ends up discarded -- keeps both the forward pass and
    # jax.grad safe from NaN (see PHI_COLLAPSE definition near top of file).
    phi1_safe = jnp.where(phi1_int > PHI_COLLAPSE, phi1_int, 1.0)
    phi2_safe = jnp.where(phi2_int > PHI_COLLAPSE, phi2_int, 1.0)

    # Hydraulic diameter per phase — Taitel-Dukler stratified segment
    # geometry, replacing the naive D*phi_i scaling. Each phase occupies
    # a circular segment (water pooled at the bottom, oil layered on top,
    # flat interface between them); wall-wetted arc length does NOT
    # shrink linearly with area -- a thin layer still touches a wide arc
    # of wall. D*phi_i badly over-penalizes whichever phase is the
    # minority. See "Switching to Taitel-Dukler for Wall Friction" for
    # the derivation and the isolated verification of this solve
    # (geometry + gradient checks both passed before this was wired in).
    #
    # phi1 = (phi_angle - sin(phi_angle)*cos(phi_angle)) / pi relates the
    # water segment's half-angle phi_angle to its area fraction phi1 --
    # transcendental, so Newton-iterate. Clamp away from 0/1 first:
    # sin(phi_angle) -> 0 at both ends, which would blow up the Newton
    # step (and its gradient) -- same reasoning as phi1_safe/phi2_safe.
    phi1_td = jnp.clip(phi1_int, PHI_COLLAPSE, 1.0 - PHI_COLLAPSE)
    phi_angle = jnp.pi * phi1_td              # initial guess: linear approx
    for _ in range(6):                        # fixed unroll -- differentiable
        f_val   = phi_angle - jnp.sin(phi_angle) * jnp.cos(phi_angle) - jnp.pi * phi1_td
        f_prime = 2.0 * jnp.sin(phi_angle)**2 + eps
        phi_angle = phi_angle - f_val / f_prime

    # Closed-form hydraulic diameters from the converged segment geometry
    # (D_h = 4*Area/wetted_perimeter for a circular segment).
    D_h1 = jnp.pi * D * phi1_safe / (phi_angle + eps)           # water hydraulic diameter [m]
    #D_h2 = jnp.pi * D * phi2_safe / (jnp.pi - phi_angle + eps)  # oil hydraulic diameter [m]
    
    # w (interfacial weight) is now a function parameter, not a hardcoded
    # constant -- see advance_momentum's signature -- so it can be treated as
    # a differentiable, learnable scalar. Default matches the last hand-tested value.
    # adding hydraulic diameter for oil phase to account for the interface between the two phases
    D_h2 = jnp.pi * D * phi2_safe / (jnp.pi - phi_angle + w * jnp.sin(phi_angle) + eps)  # oil hydraulic diameter [m]

    
    # changing back to old D_h1, D_h2 for now to debug
    #D_h1 = D * phi1_safe
    #D_h2 = D * phi2_safe

    # Reynolds number per phase -- LOCAL, per-cell/per-timestep values
    # from the actual simulated velocity field, used only for the
    # Blasius/laminar friction-factor switch below. Distinct from the
    # Re1_ref/Re2_ref function argument above (a single fixed reference
    # value per condition) -- same physical concept, deliberately
    # different names/roles, see the docstring note above. Rescaled by
    # Re1_ref/Re2_ref (multiply) instead of dividing by raw mu1/mu2,
    # now that rho1_int/u1_int/D_h1 are all dimensionless.
    # Using absolute velocity to keep Re positive regardless of flow direction
    Re1 = Re1_ref * rho1_int * jnp.abs(u1_int) * D_h1
    Re2 = Re2_ref * rho2_int * jnp.abs(u2_int) * D_h2
    ...
    friction1_two_phase = -(f_D1 / (2.0 * D_h1 + eps)) \
                * rho1_int * u1_int * jnp.abs(u1_int) * phi1_int

    friction2_two_phase = -(f_D2 / (2.0 * D_h2 + eps)) \
                * rho2_int * u2_int * jnp.abs(u2_int) * phi2_int

    # Collapsed phase contributes no independent wall friction -- its
    # momentum is carried by the dominant phase instead (see u1_star/
    # u2_star collapse in time_step / time_step_learned)
    friction1 = jnp.where(phi1_int > PHI_COLLAPSE, friction1_two_phase, 0.0)
    friction2 = jnp.where(phi2_int > PHI_COLLAPSE, friction2_two_phase, 0.0)

    # k1/k2: phase-specific friction MAGNITUDE multipliers (distinct from w,
    # which only reshapes oil's hydraulic diameter via S_i -- k1/k2 scale
    # each phase's whole friction force). Defaults of 1.0 leave physics
    # unchanged unless explicitly overridden.
    friction1 = k1 * friction1
    friction2 = k2 * friction2
    #'''
...
def solve_tridiagonal(a, b, c, d):
    ...
    # Reverse because scan went backwardsd
...
def time_step(state, dt, dx, drag_coeff, D, p_inlet, p_outlet, d_b, mu1, mu2, theta,
               phi1_inlet, Re1_ref=None, Re2_ref=None, Fr_ref=None):
    """
    ...
    mom1_new, mom2_new = advance_momentum(
        state, dt, dx, drag_coeff, d_b, D, mu1, mu2, theta,
        Re1_ref=Re1_ref, Re2_ref=Re2_ref, Fr_ref=Fr_ref
    )
    
    # Calculate intermediate velocities from momentum, with an explicit
    # single-phase collapse below PHI_COLLAPSE: a near-vanished phase is
    # treated as moving WITH the dominant phase, rather than dividing
    # momentum by an ever-smaller phi (which amplifies noise into large,
    # unstable velocities). The division itself always uses a safely-
    # substituted phi (dummy 1.0 below threshold) so it never sees a
    # near-zero denominator, even in a jnp.where branch that gets
    # discarded -- this keeps both the forward pass and jax.grad safe.
    eps = 1e-10

    phi1_safe = jnp.where(phi1_int > PHI_COLLAPSE, phi1_int, 1.0)
    phi2_safe = jnp.where(phi2_int > PHI_COLLAPSE, phi2_int, 1.0)

    u1_two_phase = mom1_new / (phi1_safe * rho1_int + eps)
    u2_two_phase = mom2_new / (phi2_safe * rho2_int + eps)

    u1_star = jnp.where(phi1_int > PHI_COLLAPSE, u1_two_phase, u2_two_phase)
    u2_star = jnp.where(phi2_int > PHI_COLLAPSE, u2_two_phase, u1_two_phase)
...
def time_step_learned(state, dt, dx, drag_c, D,
                       ...
                       theta, phi1_inlet, w=1.0, k1=1.1102, k2=0.5853,
                       Re1_ref=None, Re2_ref=None, Fr_ref=None):
    """
    ...
    Re1_ref, Re2_ref, Fr_ref: passed straight through to advance_momentum
    -- see its docstring for the nondimensional convention. Required in
    practice (default None only so old dimensional call sites still parse).
    """
    rho1_int = state['rho1'][1:-1]
    rho2_int = state['rho2'][1:-1]
    ...
    mom1_new, mom2_new = advance_momentum(
        state, dt, dx, drag_c, d_b, D, mu1, mu2, theta, w=w, k1=k1, k2=k2,
        Re1_ref=Re1_ref, Re2_ref=Re2_ref, Fr_ref=Fr_ref
    )

    # Explicit single-phase collapse below PHI_COLLAPSE -- see time_step
    # for the full explanation. Division always uses a safely-substituted
    # phi so it stays safe for jax.grad even in the discarded branch.
    eps = 1e-10
...
if __name__ == "__main__":
    ...
    # rho2_val   = 825.0   # oil [kg/m³] for Ibarra EXXSOL D140 data sret
    rho2_val = 801.0  # oil for Angeli Hewitt 1998 data set
    
    
    #phi1_0     = 0.3156   # 30% water
    phi1_0     = 0.70   # 30% water
    # defining phase composition of inlet BC when have plugging
    phi1_inlet_bc = 0.70    # ≈ 0.05
    ...
    #'''
    # Ibarra paper water and oil properties
    mu1        = 0.9E-3     # water [Pa·s]
    mu2        = 5.4E-3     # oil [Pa·s]
    L           = 8.5        # pipe length [m] — for Ibarra et al. 2015
    D           = 0.032      # pipe diameter [m] — 32mm ID, matches their test section
    rho1_val    = 998.0      # water [kg/m³]
    rho2_val    = 825.0      # oil [kg/m³] for Ibarra EXXSOL D140 data set
    ###############################################
    #'''
    
    '''
    # Angeli and Hewitt paper water and oil properties
    mu1        = 1.0E-3     # water [Pa·s]
    mu2        = 1.6E-3     # oil [Pa·s]
    L           = 9.5        # pipe length [m] — for Angeli & Hewitt 1998
    D           = 0.024      # pipe diameter [m] — 244mm
    rho1_val    = 998.0      # water [kg/m³]
    rho2_val    = 801.0      # oil [kg/m³] for Angeli & Hewitt 1998 data set
    ################################################
    '''

    '''
    # Russell 1959 paper water and oil properties
    mu1         = 0.8E-3     # water [Pa·s]
    mu2         = 0.018    # oil [Pa·s]
    L           = 8.5893      # pipe length [m] — for Russell 1959 paper
    D           = 0.020465    # pipe diameter [m] — matches their 25.4mm test section for Russell 1959 paper
    rho1_val    = 998.0   # water [kg/m³]
    rho2_val    = 834.0   # oil [kg/m³] for Russell 1959 paper
    ################################################
    '''
    ...
    # NONDIMENSIONAL CONVENTION: this demo has no experimentally-known Um
    # (starts from rest, doesn't fit a specific target) -- uses a nominal
    # Um_ref=1.0 as the reference velocity. See advance_mass's convention
    # note / generate_real_dataset for the full derivation.
    Um_ref    = 1.0
    D_star    = 1.0
    L_star    = L / D
    rho1_star = 1.0
    rho2_star = rho2_val / rho1_val
    d_b_star  = d_b / D
    P_c           = rho1_val * Um_ref**2
    p_inlet_star  = p_inlet  / P_c
    p_outlet_star = p_outlet / P_c
    Re1_ref = rho1_val * Um_ref * D / mu1
    Re2_ref = rho1_val * Um_ref * D / mu2
    Fr_ref  = Um_ref / jnp.sqrt(9.81 * D)
    ...
    print("1D Two-Phase Mixture Flow Solver")
    print("=" * 60)
    print(f"  Grid:       N={N} cells, dx*={dx:.4f} (dimensionless)")
    ...
    dt_star  = dt_fixed * Um_ref / D   # dt* = dt_seconds * Um/D
    ...
    def scan_step(state, dt):
        new_state = time_step(
            state, dt, dx, drag_coeff, D_star,
            p_inlet_star, p_outlet_star, d_b_star, mu1, mu2,
            theta, phi1_inlet_bc,
            Re1_ref=Re1_ref, Re2_ref=Re2_ref, Fr_ref=Fr_ref
        )
    ...
    def run_chunk(init_state):
        final, _ = jax.lax.scan(
            scan_step,
            init_state,
            jnp.full(save_every_window, dt_star),
        )
        return final
    ...
    print(f"  u1*={float(jnp.max(jnp.abs(spinup_state['u1']))):.4f}  "
        f"u2*={float(jnp.max(jnp.abs(spinup_state['u2']))):.4f}  (dimensionless, u* = u/Um_ref)")
